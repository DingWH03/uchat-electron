...
export async function syncContacts(): Promise<void> {
  ...
  if (!remoteRes.status || !remoteRes.data) {
    console.error('获取远程更新时间失败:', remoteRes.message)
    return
  }
  ...
  if (!listRes.status || !listRes.data) {
    console.error('获取联系人列表失败:', listRes.message)
    return
  }
    ...
    if (serverTs >= localTs) {
      ...
      try {
        ...
        if (res.status && res.data) {
          ...
          console.error(`[Sync] 群聊 ${groupId} 获取增量消息失败`)
        }
      } catch (error) {
    ...
    if (serverTs > localTs) {
      ...
      try {
        ...
        if (res.status && res.data) {
          ...
          console.error(`[Sync] 私聊 ${userId} 获取增量消息失败`)
        }
      } catch (error) {
